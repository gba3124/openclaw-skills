...
```
① OpenClaw 安裝 → ② 模型設定 → ③ LINE 外掛啟用 → ④ 帳號註冊確認（LINE Bot Webhook） → ⑤ Webhook 驗證 → ⑥ Token 瘦身優化 → ⑦ LINE Bot 串接成功並驗證
   (10 min)      (8 min)      (6 min)                       (8 min)                         (8 min)           (10 min)             (10 min)
```
...
本關遊戲化帶法（agent 使用）：
- 關卡名：`裝備鍛造`
- 任務目標：裝好 `git/node/docker`
- 過關口令：`裝備鍛造完成`
- 過關回饋：`很好，工具到位，接下來就能召喚專案本體。`
